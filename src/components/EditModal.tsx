...
const normalizeSize = (size: any) => ({
  sizeValue: size.sizeValue || size.size || "",
  price: toNumber(size.price),
  oldPrice: size.oldPrice ? toNumber(size.oldPrice) : null,
});

const normalizeColor = (color: any) => ({
  colorName: color.colorName || color.name || "Standart",
  colorCode: color.colorCode || color.code || color.hex || "#cccccc",
...
const EditModal = ({ product, onClose, onSave }: EditModalProps) => {
  const [formData, setFormData] = useState({
    ...product,
    status: product.isDiscount ? "Endirimli" : product.isPopular ? "Popular" : product.status || "Standart",
    partType: product.partType || product.fabric || "",
    sizeOptions: product.sizeOptions?.length
      ? product.sizeOptions
      : product.sizes?.length
        ? product.sizes
        : [{ sizeValue: "", price: "", oldPrice: "" }],
  ...
  const handleSubmit = () => {
    ...
    if (!payload.name || payload.sizeOptions.length === 0) return;
    onSave(payload);
  };
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 lg:p-10 overflow-hidden">
      ...
      <div className="bg-white rounded-[3rem] p-8 lg:p-12 w-full max-w-4xl relative shadow-2xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-10 duration-500 z-10 custom-scrollbar">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full border-2 border-slate-100 p-4 rounded-2xl focus:border-[#C5A059] outline-none transition-all font-medium"
              />
            </div>
            <div className="space-y-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full border-2 border-slate-100 p-4 rounded-2xl focus:border-[#C5A059] outline-none min-h-[150px] resize-none font-medium leading-relaxed"
              />
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  name="room"
                  value={formData.room || formData.roomType || ""}
                  onChange={handleChange}
                  className="w-full border-2 border-slate-100 p-4 rounded-2xl outline-none font-bold text-[#0A1128] cursor-pointer"
                >
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase text-slate-400 tracking-wider">Parça Növü</label>
              <input
                name="partType"
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4 shadow-inner mt-10">
          ...
          {formData.sizeOptions.map((s: any, index: number) => (
            <div key={index} className="flex gap-3 items-center">
              <input
                value={s.sizeValue || s.size || ""}
                onChange={(e) => handleSizeChange(index, "sizeValue", e.target.value)}
                placeholder="Ölçü"
                className="flex-1 p-4 rounded-2xl border-0 shadow-sm outline-none font-medium bg-white"
              />
              <input
                value={s.price || ""}
                onChange={(e) => handleSizeChange(index, "price", e.target.value)}
                placeholder="Qiymət"
                className="w-28 p-4 rounded-2xl border-0 shadow-sm outline-none font-bold text-[#0A1128] bg-white"
              />
              <input
                value={s.oldPrice || ""}
                onChange={(e) => handleSizeChange(index, "oldPrice", e.target.value)}
                placeholder="Köhnə"
                className="w-32 p-4 rounded-2xl border-0 shadow-sm outline-none text-slate-400 italic bg-white"
              />
              <button
                onClick={() =>
                  setFormData({
                    ...formData,
                    sizeOptions: formData.sizeOptions.filter((_: any, idx: number) => idx !== index),
                  })
                }
                className="p-2 text-rose-500 hover:bg-rose-100 rounded-xl transition-all shrink-0"
              >
                <Trash2 size={20} />
              </button>
            </div>
          ...
          <button
            onClick={() =>
              setFormData({
                ...formData,
                sizeOptions: [...formData.sizeOptions, { sizeValue: "", price: "", oldPrice: "" }],
              })
            }
            className="text-xs font-black text-[#C5A059] tracking-widest hover:underline px-2 pt-2"
          >
            + YENİ ÖLÇÜ ƏLAVƏ ET
          </button>
        </div>
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4 shadow-inner mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {formData.colors.map((color: any, index: number) => (
              <div key={index} className="flex items-center gap-3 p-3 bg-white rounded-2xl shadow-sm border border-slate-100">
                <input
                  type="color"
                  value={color.colorCode || color.code || color.hex || "#ffffff"}
                  onChange={(e) => handleColorChange(index, "colorCode", e.target.value)}
                  className="w-10 h-10 rounded-xl cursor-pointer border-0 shrink-0"
                />
                <input
                  value={color.colorName || color.name || ""}
                  onChange={(e) => handleColorChange(index, "colorName", e.target.value)}
                  placeholder="Rəng adı"
                  className="flex-1 text-sm outline-none font-medium text-[#0A1128]"
                />
                <input
                  type="file"
                  className="hidden"
                ...
                <button
                  onClick={() =>
                    setFormData({
                      ...formData,
                      colors: formData.colors.filter((_: any, idx: number) => idx !== index),
                    })
                  }
          ...
          <button
            onClick={() =>
              setFormData({
                ...formData,
                colors: [...formData.colors, { colorCode: "#ffffff", colorName: "", mainImage: null }],
              })
            }
            className="w-full py-4 border-2 border-dashed border-slate-200 text-slate-400 font-bold rounded-2xl hover:border-[#C5A059] hover:text-[#C5A059] transition-all bg-white/50"
          >
            + Yeni Rəng Sahəsi
          </button>
        </div>
