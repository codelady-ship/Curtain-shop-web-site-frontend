...
const BasketAndFavorite = () => {
  ...
  const [dbProducts, setDbProducts] = useState([]); // Bazadan gələn məhsullar

  // 1. Məhsulları bazadan çək
  useEffect(() => {
    fetchProducts()
      .then((res) => {
  ...
  return (
    <div className="bg-[#FBFBFA] min-h-screen flex flex-col">
      ...
      <main className="flex-grow pt-12 pb-20">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center mb-12">
            <h1 className="text-3xl md:text-4xl font-serif text-slate-900 italic">
              Mənim Seçimlərim
            </h1>
                    ...
                    wishlistProducts.map((product) => (
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-1">
                          <h4 className="font-serif text-sm text-slate-800 leading-tight">
                            {product.name}
                          </h4>
                ...
                <AnimatePresence mode="popLayout">
                  {cartItems.length > 0 ? (
                    <>
                            ...
                            <div className="flex-1 space-y-2">
                              <div>
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                                  {item.selectedColor?.colorName || item.selectedColor?.name || "Standart"}{" "}
                                  • {item.selectedSize?.sizeValue || item.selectedSize?.size || "Standart"}{" "}
                                  sm
                                </p>
                              </div>
                              <button
                      ...
                      <motion.div
                        layout
                        className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-6"
                      >
                        <div className="flex justify-between items-end border-b border-gray-50 pb-6">
                          <div className="space-y-1">
                            ...
                            <h3 className="text-4xl font-black text-slate-900">
                              {subtotal.toFixed(2)}{" "}
                              <span className="text-[#C5A059] italic text-2xl ml-1">
                                ₼
                              </span>
                            </h3>
                          </div>
                          <div className="text-green-500 text-[10px] font-black bg-green-50 px-3 py-1 rounded-full uppercase">
                            Ödənişə hazırdır
                          </div>
                        </div>
                        <button
                    ...
                    <div className="bg-white p-20 rounded-[3rem] border border-gray-50 flex flex-col items-center justify-center text-center space-y-6">
                      ...
                      <p className="text-slate-900 font-serif text-xl italic">
                        Səbətiniz hələ ki, boşdur
                      </p>
                      <button
                        onClick={() => navigate("/products")}
                        className="bg-black text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#C5A059]"
                      >
                        Alış-verişə başla
                      </button>
                    </div>
      ...
      <footer className="bg-black text-white py-6 border-t border-white/5 mt-auto">
        <div className="container mx-auto px-6 text-center">
          <h3 className="font-serif italic text-3xl text-[#C5A059]">
            Properde
          </h3>
          <p className="text-[8px] tracking-[0.5em] text-gray-600 uppercase mt-1 italic">
            {" "}
            By Premium Tekstil
          </p>
